...
function UsageModesTabs({ modes, variant = "card", isActive = true }) {
  ...
  useEffect(()=> {
    console.log('os modes recebidos são esses ',modes)
  },[modes])

  const [activeId, setActiveId] = useState(safeModes?.[0]?.id);
  ...
  // Garante que, se a lista de modos mudar (ex: troca de variante/produto),
  // o modo ativo seja sempre um id válido da nova lista.
  useEffect(() => {
    if (safeModes.length === 0) return;
    const stillExists = safeModes.some((m) => m.id === activeId);
    if (!stillExists) {
      setActiveId(safeModes[0].id);
    }
  ...
  const activeMode = safeModes.find((m) => m.id === activeId) || safeModes[0];
