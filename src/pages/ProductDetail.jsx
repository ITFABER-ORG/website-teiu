...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const onWheelReverse = (e) => {
      if (isAnimationDone && e.deltaY < 0 && window.scrollY <= 10) {
        e.preventDefault();
        setIsReversing(true);
        setIsAnimationDone(false);
      }
    ...
    window.addEventListener("wheel", onWheelReverse, { passive: false });

    return () => window.removeEventListener("wheel", onWheelReverse);
  }, [isAnimationDone]);
  ...
  useEffect(() => {
    ...
    const onWheel = (e) => {
      if (!isAnimationDone || isReversing) {
        ...
        let next =
          progressRef.current + direction * Math.abs(e.deltaY) * SPEED;
